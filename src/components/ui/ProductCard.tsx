...
const ProductCard = ({key, title, price, newPrice, imageSrc}: ProductCardProps) => {
  return (
    <div
    key={key}
    className="relative aspect-square w-full overflow-hidden rounded-[16px] bg-[#F5F5F7] p-[16px]"
    >
    <div className="relative z-10 flex items-center justify-between gap-2">
        ...
        <Link
            to="/favourites"
            className="flex size-8 shrink-0 items-center justify-center rounded-full border border-[#CECECF]"
        ...
        <Link
            to="/cart"
            className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[#0C0C0C]"
    ...
    <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 items-center whitespace-nowrap">
       {newPrice && (
        ...
        <span className={cn(
            ...
            newPrice ? 'ml-1 rounded-[5px] bg-[#0C0C0C] opacity-56 px-1.5 py-[4px] text-[11px] leading-none text-white line-through' : 'bg-[#0C0C0C] text-white'
        )}>
        {price}₴
        </span>
    </div>
